...
def get_venda_parcial_recibo(venda_id):
    """Recibo completo de uma venda parcial, com saldo anterior e restante do lote."""
    venda = fetchone("""
        SELECT v.*, p.nome pessoa_nome, p.documento, p.telefone, p.whatsapp, p.cidade, p.fazenda,
               c.quantidade_sacas compra_sacas, c.valor_saca compra_valor_saca, c.data compra_data,
               c.lote compra_lote, c.tipo_cafe compra_tipo, c.peso_kg compra_peso_kg, c.origem_cafe compra_origem
        FROM vendas v
        LEFT JOIN pessoas p ON p.id=v.pessoa_id
        LEFT JOIN compras c ON c.id=v.compra_id
        WHERE v.id=?
    """, (venda_id,))
    if not venda:
        return None
    compra_id = venda['compra_id']
    comprado = float(venda['compra_sacas'] or 0) if compra_id else 0
    vendido_antes = 0
    restante_lote = 0
    if compra_id:
        vendido_antes_row = fetchone("""
            SELECT COALESCE(SUM(quantidade_sacas),0) v
            FROM vendas
            WHERE compra_id=? AND (data < ? OR (data = ? AND id < ?))
        """, (compra_id, venda['data'], venda['data'], venda_id))
        vendido_antes = float(vendido_antes_row['v'] or 0) if vendido_antes_row else 0
        restante_row = fetchone("""
            SELECT COALESCE(c.quantidade_sacas,0) - COALESCE((SELECT SUM(v.quantidade_sacas) FROM vendas v WHERE v.compra_id=c.id),0) estoque
            FROM compras c WHERE c.id=?
        """, (compra_id,))
        restante_lote = float(restante_row['estoque'] or 0) if restante_row else 0
    vendido_agora = float(venda['quantidade_sacas'] or 0)
    saldo_anterior = max(0, comprado - vendido_antes) if compra_id else 0
    saldo_depois = max(0, saldo_anterior - vendido_agora) if compra_id else 0
    lote_nome = venda['compra_lote'] or venda['lote'] or (f"Lote {compra_id}" if compra_id else '-')
    tipo = venda['tipo_cafe'] or venda['compra_tipo'] or '-'
    texto = f"""Café Boa Vista\nRecibo de venda parcial do cliente\n\nCliente: {venda['pessoa_nome'] or 'Sem cadastro'}\nLote: {lote_nome}\nTipo: {tipo}\nData da venda: {venda['data']}\n\nSaldo antes da venda: {br_num(saldo_anterior)} sacas\nVendido agora: {br_num(vendido_agora)} sacas\nPreço/saca: {br_money(venda['valor_saca'])}\nTotal da venda: {br_money(venda['valor_total'])}\nSaldo após a venda: {br_num(saldo_depois)} sacas\n\nPagamento ao cliente: {venda['status_recebimento'] or '-'}"""
    return dict(venda=venda, comprado=comprado, vendido_antes=vendido_antes,
                vendido_agora=vendido_agora, saldo_anterior=saldo_anterior,
                saldo_depois=saldo_depois, restante_lote=restante_lote,
                lote_nome=lote_nome, tipo=tipo, texto_whatsapp=texto, hoje=today())
...
def modo_rapido():
    ...
    hoje = today()
    provas_hoje = fetchall("""SELECT pr.*, COALESCE(p.nome, pr.nome_avulso, pr.nome_cliente, '-') cliente_nome
                              FROM provas pr LEFT JOIN pessoas p ON p.id=pr.pessoa_id
                              WHERE pr.data=? ORDER BY pr.id DESC LIMIT 8""", (hoje,))
    compras_hoje = fetchall("""SELECT c.*, p.nome pessoa_nome, ({expr}) estoque_lote
                               FROM compras c LEFT JOIN pessoas p ON p.id=c.pessoa_id
                               WHERE c.data=? ORDER BY c.id DESC LIMIT 8""".format(expr=lote_estoque_expr()), (hoje,))
    vendas_hoje = fetchall("""SELECT v.*, p.nome pessoa_nome
                              FROM vendas v LEFT JOIN pessoas p ON p.id=v.pessoa_id
                              WHERE v.data=? ORDER BY v.id DESC LIMIT 8""", (hoje,))
    valores_hoje = fetchall("""SELECT a.*, p.nome pessoa_nome FROM adiantamentos a
...
def achar_ou_criar_pessoa_rapida(nome, whatsapp='', documento=''):
    ...
    if not nome:
        return None
    existente = fetchone("SELECT id FROM pessoas WHERE LOWER(nome)=LOWER(?) LIMIT 1", (nome,))
    if existente:
        return existente['id']
    con = db(); cur = con.cursor()
...
def rapido_deposito():
    ...
    if request.method == 'POST':
        ...
        if not pessoa_id:
            pessoa_id = achar_ou_criar_pessoa_rapida(request.form.get('nome_rapido'), request.form.get('whatsapp_rapido'), request.form.get('documento_rapido'))
        ...
        if not qtd:
            ...
            return redirect(url_for('rapido_deposito'))
        con = db(); cur = con.cursor()
        cur.execute("""INSERT INTO compras (pessoa_id,data,lote,tipo_cafe,quantidade_sacas,peso_kg,divisor_saca,ajuste_sacas,valor_saca,valor_total,status_pagamento,forma_pagamento,frete,outras_despesas,origem_cafe,status_lote,observacao)
                     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (pessoa_id, request.form['data'], request.form.get('lote'), request.form.get('tipo_cafe'), qtd, peso, divisor, ajuste, valor_saca, total, 'Pendente', request.form.get('forma_pagamento'), 0, 0, request.form.get('origem_cafe'), 'Em estoque', request.form.get('observacao')))
        ...
        cur.execute("INSERT INTO financeiro (data,tipo,descricao,categoria,valor,status,origem) VALUES (?,?,?,?,?,?,?)",
                    (request.form['data'], 'Saída', f"Café em depósito/compra - {lote}", 'Compra de café', total, 'Pendente', f"compra:{compra_id}"))
        con.commit(); con.close()
        log_acao('Café em depósito rápido', 'compra', compra_id, f"{lote} - {br_num(qtd)} sacas")
        flash(f'Café em depósito lançado: {br_num(qtd)} sacas. Agora já aparece no saldo do cliente.')
        return redirect(url_for('editar_compra', compra_id=compra_id))
    return render_template('rapido_deposito.html', pessoas=pessoas_rows, hoje=today(), tipos_cafe=TIPOS_CAFE)


@app.route('/rapido/vender', methods=['GET','POST'])
def rapido_vender():
    update_all_status()
    ...
    if request.method == 'POST':
        ...
        if not compra_id:
            ...
            return redirect(url_for('rapido_vender'))
        ...
        cur.execute("""INSERT INTO vendas (pessoa_id,compra_id,data,lote,tipo_cafe,quantidade_sacas,valor_saca,subtotal_cafe,juros_percentual,meses_atraso,valor_juros,valor_total,custo_saca,lucro_total,status_recebimento,forma_pagamento,observacao)
                     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (compra['pessoa_id'], compra_id, request.form['data'], compra['lote'], compra['tipo_cafe'], qtd, valor_saca, subtotal, 0, 0, 0, subtotal, custo_saca, lucro, status_venda, request.form.get('forma_pagamento'), request.form.get('observacao')))
        venda_id = cur.lastrowid
        cur.execute("INSERT INTO financeiro (data,tipo,descricao,categoria,valor,status,origem) VALUES (?,?,?,?,?,?,?)",
                    (request.form['data'], 'Saída', f"Pagamento de venda de café ao cliente - {compra['lote'] or venda_id}", 'Pagamento ao cliente', subtotal, status_financeiro, f"venda:{venda_id}"))
        ...
        log_acao('Venda rápida lançada', 'venda', venda_id, f"{compra['pessoa_nome'] or ''} - {br_num(qtd)} sacas")
        flash(f'Venda lançada. Restam {br_num(restante)} sacas nesse lote.')
        return redirect(url_for('recibo_venda_parcial', venda_id=venda_id))
    return render_template('rapido_vender.html', lotes=lotes, hoje=today())
...
def depositos():
    ...
    params = []
    where = [f"({lote_estoque_expr()}) > 0.0001"]
    if q:
        like = f"%{q}%"
        where.append("(p.nome LIKE ? OR c.lote LIKE ? OR c.origem_cafe LIKE ? OR c.tipo_cafe LIKE ?)")
        params += [like, like, like, like]
    if tipo:
        where.append("COALESCE(c.tipo_cafe,'') = ?")
        params.append(tipo)
    where_sql = " AND ".join(where)
    rows = fetchall(f"""
        SELECT c.*, p.nome pessoa_nome, p.telefone, p.whatsapp, p.documento,
               ({lote_estoque_expr()}) estoque_lote,
               COALESCE((SELECT SUM(v.quantidade_sacas) FROM vendas v WHERE v.compra_id=c.id),0) vendido,
               COALESCE((SELECT SUM(v.valor_total) FROM vendas v WHERE v.compra_id=c.id),0) valor_vendido
        FROM compras c
        ...
        ORDER BY p.nome COLLATE NOCASE, c.data DESC, c.id DESC
    """, params)
    total_sacas = sum(float(r['estoque_lote'] or 0) for r in rows)
    valor_estimado = sum(float(r['estoque_lote'] or 0) * float(r['valor_saca'] or 0) for r in rows)
    total_vendido = sum(float(r['valor_vendido'] or 0) for r in rows)
    return render_template('depositos.html', depositos=rows, q=q, tipo=tipo, tipos_cafe=TIPOS_CAFE,
                           total_sacas=total_sacas, valor_estimado=valor_estimado, total_vendido=total_vendido)
...
@app.route('/vendas/<int:venda_id>/recibo-parcial')
def recibo_venda_parcial(venda_id):
    dados = get_venda_parcial_recibo(venda_id)
    if not dados:
        flash('Venda não encontrada.')
        return redirect(url_for('vendas'))
    return render_template('recibo_venda_parcial.html', **dados)
